fun main() {
    ...
    val cards_tipe: String = "Mastercard" //"Visa" "Мир"


    if ((curent_amount + prev_amounts_in_day) <= limit_day && (curent_amount + prev_amounts_in_mounth) <= limit_month) {
        val realTax = getTax(cards_tipe, prev_amounts_in_mounth, curent_amount)
        if (realTax >= 0) {
...
fun get_tax_mastercard(prev_amounts_in_mounth: Int = 0, curent_amount: Int): Int {
    return if (curent_amount <= 75000) {
        0
    } else ((curent_amount + prev_amounts_in_mounth - 75000) * 0.006 + 20).toInt()
}
